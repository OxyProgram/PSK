...
def go():
    ...
    try:
        ...
        tkinter.messagebox.showwarning("Alert", "Computer not Connected to internet")
        raise
    except ValueError:
        print ('Entered')
        tkinter.messagebox.showwarning("Alert", "Server Error")
        raise
    except:
        print ("invalid")
        tkinter.messagebox.showwarning("Alert", "Invalid City!!")
        raise
    print (city)
...
#using API
    tmp = round(dic['today']['temp'])
    tmp_min = round(dic['today']['min'])
    tmp_max = round(dic['today']['max'])
    t = tmp
    tmp = str(tmp) + "°C"
...
#current weather icon
    ...
    mainTemp = canvas.create_text(screen_width/2,screen_height*6*0.066667,text=tmp,fill="white",font="Sans 40 ")
    mainTemp_min = canvas.create_text(screen_width*0.066667*4,screen_height*5.5*0.066667,text=tmp_min,fill="white",font="Sans 30 ")
    mainTemp_max = canvas.create_text(screen_width*0.066667*11,screen_height*5.5*0.066667,text=tmp_max,fill="white",font="Sans 30 ")
...
#min and max temp for future four days
    labelMin = canvas.create_text(screen_width/5.5,screen_height*11*0.066667,text="Min °C",fill="white",font="Sans 17 bold")
    labelMax = canvas.create_text(screen_width/5.5,screen_height*11.5*0.066667,text="Max °C",fill="white",font="Sans 17 bold")

    day1_min = canvas.create_text(screen_width/3.8,screen_height*11*0.066667,text=str(round(dic[1]['min'])) + "°C",fill="white",font="Sans 17 bold")
    day2_min = canvas.create_text(screen_width/2.3,screen_height*11*0.066667,text=str(round(dic[2]['min'])) + "°C",fill="white",font="Sans 17 bold")
    day3_min = canvas.create_text(screen_width/1.65,screen_height*11*0.066667,text=str(round(dic[3]['min'])) + "°C",fill="white",font="Sans 17 bold")
    day4_min = canvas.create_text(screen_width/1.3,screen_height*11*0.066667,text=str(round(dic[4]['min'])) + "°C",fill="white",font="Sans 17 bold")
    day1_max = canvas.create_text(screen_width/3.8,screen_height*11.5*0.066667,text=str(round(dic[1]['max'])) + "°C",fill="white",font="Sans 17 bold")
    day2_max = canvas.create_text(screen_width/2.3,screen_height*11.5*0.066667,text=str(round(dic[2]['max'])) + "°C",fill="white",font="Sans 17 bold")
    day3_max = canvas.create_text(screen_width/1.65,screen_height*11.5*0.066667,text=str(round(dic[3]['max'])) + "°C",fill="white",font="Sans 17 bold")
    day4_max = canvas.create_text(screen_width/1.3,screen_height*11.5*0.066667,text=str(round(dic[4]['max'])) + "°C",fill="white",font="Sans 17 bold")

    button = tkinter.Button(canvas, relief=tkinter.RIDGE, borderwidth=2, bg="gray", highlightbackground="gray", highlightcolor="gray", font=("Comic sans ms",10,"bold"), width=100,height=30)

# Function to be called when the button is clicked
# fahrenheit = celsius * 1.8 + 32
    def button_click():

        if button.cget("text") == "Switch to Fahrenheit":
            button.config(text="Switch to Celsius")
            changeToFahrenheit()
        
        else:
            button.config(text="Switch to Fahrenheit")
            changeToCelsius()

    def changeToFahrenheit():
        tmp_f = round(dic['today']['temp'] * 1.8 + 32)
        tmp_min_f = round(dic['today']['min'] * 1.8 + 32)
        tmp_max_f = round(dic['today']['max'] * 1.8 + 32)
        tmp_f = str(tmp_f) + "°F"
        tmp_min_f = "Min : " + str(tmp_min_f) + "°F"
        tmp_max_f = "Max : " + str(tmp_max_f) + "°F"
        canvas.itemconfig(mainTemp, text=tmp_f)
        canvas.itemconfig(mainTemp_max, text=tmp_max_f)
        canvas.itemconfig(mainTemp_min, text=tmp_min_f)

        canvas.itemconfig(labelMin, text= "Min °F")
        canvas.itemconfig(labelMax, text= "Max °F")
        canvas.itemconfig(day1_min, text= str(round(dic[1]['min'] * 1.8 + 32)) + "°F" )
        canvas.itemconfig(day2_min, text= str(round(dic[2]['min'] * 1.8 + 32)) + "°F" )
        canvas.itemconfig(day3_min, text= str(round(dic[3]['min'] * 1.8 + 32)) + "°F" )
        canvas.itemconfig(day4_min, text= str(round(dic[4]['min'] * 1.8 + 32)) + "°F" )
        canvas.itemconfig(day1_max, text= str(round(dic[1]['max'] * 1.8 + 32)) + "°F" )
        canvas.itemconfig(day2_max, text= str(round(dic[2]['max'] * 1.8 + 32)) + "°F" )
        canvas.itemconfig(day3_max, text= str(round(dic[3]['max'] * 1.8 + 32)) + "°F" )
        canvas.itemconfig(day4_max, text= str(round(dic[4]['max'] * 1.8 + 32)) + "°F" )

    def changeToCelsius():
        tmp_c = round(dic['today']['temp'])
        tmp_min_c = round(dic['today']['min'])
        tmp_max_c = round(dic['today']['max'])
        tmp_c = str(tmp_c) + "°C"
        tmp_min_c = "Min : " + str(tmp_min_c) + "°C"
        tmp_max_c = "Max : " + str(tmp_max_c) + "°C"
        canvas.itemconfig(mainTemp, text=tmp_c)
        canvas.itemconfig(mainTemp_max, text=tmp_max_c)
        canvas.itemconfig(mainTemp_min, text=tmp_min_c)

        canvas.itemconfig(labelMin, text= "Min °C")
        canvas.itemconfig(labelMax, text= "Max °C")

        canvas.itemconfig(day1_min, text= str(round(dic[1]['min'])) + "°C" )
        canvas.itemconfig(day2_min, text= str(round(dic[2]['min'])) + "°C" )
        canvas.itemconfig(day3_min, text= str(round(dic[3]['min'])) + "°C" )
        canvas.itemconfig(day4_min, text= str(round(dic[4]['min'])) + "°C" )
        canvas.itemconfig(day1_max, text= str(round(dic[1]['max'])) + "°C" )
        canvas.itemconfig(day2_max, text= str(round(dic[2]['max'])) + "°C" )
        canvas.itemconfig(day3_max, text= str(round(dic[3]['max'])) + "°C" )
        canvas.itemconfig(day4_max, text= str(round(dic[4]['max'])) + "°C" )

#button for switching degree type
    button = tkinter.Button(canvas, text="Switch to Fahrenheit", command=button_click)
    canvas.create_window(screen_width/1.1,screen_height*1*0.066667, window=button)
